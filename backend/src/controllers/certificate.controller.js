...
const createCertificate = asyncHandler(async (req, res) => {
    ...
    const registration = await Registration.findOne({ user: userId, event: eventDetails._id });
    if (!registration) {
        throw new ApiError(403, "User not registered for this event");
    }

    let existing = await Certificate.findOne({ user: userId, event: eventDetails._id });
    if (existing) {
    ...
    if (eventDetails.image) {
        try {
            const response = await axios.get(eventDetails.image, { responseType: "arraybuffer" });
    ...
    doc
        .fontSize(18)
        .fillColor("#555555")
        .text(`Organized by: ${eventDetails.organizingClub}`, { align: "center" });
    ...
    doc
        .fontSize(14)
        .fillColor("#555555")
        .text(`Date: ${new Date().toLocaleDateString()}`, 50, doc.page.height - 80, { align: "left" });

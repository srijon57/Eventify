...
const sendEventEmail = asyncHandler(async (req, res) => {
    ...
    const eventDetails = await Event.findById(event).populate("createdBy", "username email");
    if (!eventDetails) {
    ...
    <p style="font-size: 16px; color: #555;"><strong>Description:</strong> ${eventDetails.description}</p>
    
    <p style="font-size: 16px; color: #555;"><strong>📍 Location:</strong> ${eventDetails.location}</p>
    
    <p style="font-size: 16px; color: #555;"><strong>🕒 Time:</strong> ${eventDetails.eventTime.toLocaleString()}</p>
    
    <p style="font-size: 16px; color: #555;"><strong>👥 Organized by:</strong> ${eventDetails.organizingClub}</p>

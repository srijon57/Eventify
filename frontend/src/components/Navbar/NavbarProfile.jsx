// src/components/Navbar/NavbarProfile.jsx
...
  // This check needs to be at the very beginning of the component's render logic
  // to prevent accessing properties of a null 'user' object.
  if (!user) {
    return null;
  }
...
      {/* Profile Image with a robust fallback check */}
      <img
        src={user.profileImage || `https://placehold.co/40x40?text=${user.username ? user.username.charAt(0).toUpperCase() : '?'}`}
        alt="Profile"
...
        {/* Use includes() for flexible role checking */}
        {user.role && user.role.toLowerCase().includes("user") && (
          <Link
            to="/studentdashboard"
            className="block px-4 py-2 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700"
            onClick={() => setOpen(false)}
